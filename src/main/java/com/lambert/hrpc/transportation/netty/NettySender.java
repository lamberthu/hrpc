...
import com.lambert.hrpc.transportation.Sender;
            ...
            bootstrap.group(group).channel(NioSocketChannel.class)
                .handler(new ChannelInitializer<SocketChannel>() {
                    ...
                    public void initChannel(SocketChannel channel) throws Exception {
                        channel.pipeline()
                            .addLast(new Encoder(Request.class)) // 将 RPC 请求进行编码（为了发送请求）
                            .addLast(new Decoder(Response.class)) // 将 RPC 响应进行解码（为了处理响应）
